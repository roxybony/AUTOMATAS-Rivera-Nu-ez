...
public class ProyFinal {
	...
	String Caux = "Id	int	float	char	,	;	+	-	*	/	(	)	=	==	<	<=	>	>=	!=	{	}	\"	palabra	Num	PRINT	SCAN	BEGIN	IF	DO	WHILE	END	ENDIF	THEN	$	S	B	P	V	PD	M	E	T	F	C";
	...
	public ProyFinal() {
		Filas = Faux.split("	");
		Columnas = Caux.split("	");
		System.out.println(Filas[18]);
		cadena = "BEGIN int a , c ; c = 1 + 0 ; SCAN a DO { PRINT c c = c + 1 ; } WHILE ( a > c ) ; IF ( a == c ) { PRINT \" \"Mensaje\" \" } END $";	
	}
		...
		do {
			...
			if (TablaBuena.elementAt(f)[c].charAt(0) == 'I') { //Verifica que sea un estado
				if (CompEntr(TipDat, cadenaS[i]) != -1) {
					...
					if (caracter.equalsIgnoreCase("print")) {
						CodObj += "printf(";
					} else {
						if(BanAux) {
							BanAux = false;
							if(caracter.equalsIgnoreCase("id")) {
								CodObj += "\"%d\"," + cadenaS[i];
							}
							//CodObj += cadenaS[i];
						} else {
							 if(caracter.equalsIgnoreCase("palabra")) {
									CodObj += "" + cadenaS[i] + "";
								}
							CodObj += ");\n";
							BanAux = true;
					...
					if (BanAsig) {
						...
						if ((Dicc.get("Var" + ContV).tipo).equals(Dicc.get(idAsigna).tipo)) {
							...
							if(CodObj_Asig.length() < 7)
							CodObj += idAsigna + " " + CodObj_Asig;
							else
								//CodObj += CodObj_Asig.length() + "";
								CodObj += CodObj_Asig;
							CodObj_Asig = "";
					...
					PosFijaT(cadenaS[i]);
				} else if (caracter.equalsIgnoreCase("id")) {
				...
				if(TablaBuena.elementAt(f)[c].equals("P21")) {
					do { 
						PSintactica.pop();
	...
	public int SearchFil() {
		String faux = PSintactica.elementAt(PSintactica.size() - 1);
